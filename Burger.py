            ...
            if(ran < 3):  # slow him down cuz he hella scary when he's fast
                ...
                if (self.direction == 0
                   and self.rect.y <= self.yboundb):  # down key
                    ...
                    self.face = 'd'
                # up key
                elif (self.direction == 1 and self.rect.y >= self.yboundt):
                        self.rect.y -= move_dist  # move up
                        self.face = 'u'
                # right key
                elif (self.direction == 2 and self.rect.x <= self.xboundr):
                    self.rect.x += move_dist  # move right
                    self.face = 'r'
                # left key
                elif (self.direction == 3 and self.rect.x >= self.xboundl):
                    self.rect.x -= move_dist  # move left
                    self.face = 'l'
